...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/gbiz_company_search \
  -H 'Content-Type: application/json' \
  -d '{"name":"トヨタ自動車","limit":3}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/gbiz_company_search`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
